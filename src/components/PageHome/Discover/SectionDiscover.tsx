...
export function SectionDiscover() {
    ...
    return (
        <section className=' bg-gray50 @tablet:bg-white mt-[120px] @tablet:mt-0'  >

            <Container className='py-[88px] grid grid-row'>
                ...
                <div className='w-full justify-between items-start hidden @Desktop:flex '>
                    ...
                    <div className='w-full max-w-[628px] '>
                        <AccordionFAQ data={faqData} onValueChange={handleAccordionValueChange} defaultValue="item-1" />
                    </div>
                    ...
                    {discoverContent.map((sectionData: DiscoverContentData, index: number) => (
                        ...
                        index === activeContentIndex && (
                            <div key={index} className='max-w-[520px] h-auto  '>
                                ...
                                <div className='flex flex-col  '>
                                    {sectionData.items.map((item: DiscoverItem, itemIndex: number) => (
                                        <div className='flex justify-between ' key={itemIndex}>
                                            ...
                                            <div className={`${index === 1 ? 'w-full' : 'max-w-[390px] h-[20px]'} flex items-start  `}>
                                                ...
                                                {item.icon && (
                                                    <div className={`${item.icon ? 'mr-4' : 'mr-0 '} `} >
                                                        <Icon className='' name={item.icon as any} />
                                                    </div>
                                                )}
                                                <p className='TypographyPinter16w500g950 '>{item.text}</p>
                                            </div>
                                    ...
                                    <p className='w-[420px] TypographyPinter16w400 pt-4'>{sectionData.finalDescription}</p>
                                </div>
                            </div>
                        )
                    ))}
