    ...
    return (
        ...
        <div className='w-full flex justify-start items-center '>
            <div className='w-full  '>
                ...
                <Accordion type="single" collapsible className="w-full h-auto border-b  " onValueChange={handleAccordionValueChange} defaultValue="item-1">
                    {faqData.map((item, index) => (
                        // Cada item do Accordion mapeado a partir dos dados do FAQ
                        <AccordionItem value={`item-${index + 1}`} key={index} className="w-full py-[16px] px-2 ">
                            {/* Trigger (cabeçalho) do item do accordion */}
                            <AccordionTrigger className="TypographyPlato24 @tablet:TypographyPlato20 pt-0 pb-[8px] hover:text-redSTD w-full ">{item.question}</AccordionTrigger>
                                ...
                                {index === activeContentIndex && discoverContent[index] && (
                                    <div key={index} className='w-full mt-4 '>
                                        ...
                                        <div className='w-full h-auto mb-[24px]'>
                                            <img src={(discoverContent[index] as DiscoverContentData).imageSrc} alt="Discover Image" className="w-full object-cover " />
                                        </div>

                                        {/* Div para a lista de itens (ícone + texto) e descrição */}
                                        <div className='flex flex-col'>
                                            {(discoverContent[index] as DiscoverContentData).items.map((discoverItem: DiscoverItem, itemIndex: number) => (
                                            ...
                                            <p className='max-w-[420px] TypographyPinter16w400 pt-4'>{(discoverContent[index] as DiscoverContentData).finalDescription}</p>
                                        </div>
                                    </div>
                                )}
                            </AccordionContent>
